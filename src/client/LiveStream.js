...
exports.StreamStartOrStopOrReset = async function({name, op}) {
  try {
    ...
    try {
      ...
      // Check for unsupported input conditions before returning
      // generic "LRO start failed - must create a stream first" error
      try {
        const configMeta = await this.ContentObjectMetadata({
          libraryId: status.libraryId,
          objectId: status.objectId,
          metadataSubtree: "live_recording_config",
          select: [
        ...
        const probeInfo = configMeta?.probe_info || configMeta?.input_stream_info;
        const unsupportedAudio = (probeInfo?.streams || [])
          .find(stream => stream.codec_type === "audio" && stream.sample_rate === 44100);

        if(unsupportedAudio) {
          return {
            state: status.state,
            error: "Unsupported audio sample rate: 44.1 kHz. Only 48 kHz audio is supported."
          };
        }
      } catch(probeError) {
        console.error("Unable to check audio sample rate", probeError);
      }
