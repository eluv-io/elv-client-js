...
class MediaIngest extends Utility {
  ...
  async body() {
    ...
    const s3Reference = Boolean(this.args.s3Reference);
    const useS3 = Boolean(this.args.s3Copy || s3Reference);
    const access = useS3
      ? this.concerns.CloudFile.credentialSet()
      : undefined;

    let fileHandles = [];
    const fileInfo = useS3
      ? this.concerns.CloudFile.fileInfo()
      : this.concerns.LocalFile.fileInfo(fileHandles);
    const fileConcern = useS3
      ? this.concerns.CloudFile
      : this.concerns.LocalFile;
    ...
    const copy = !s3Reference;
    const encrypt = !s3Reference;
    ...
    if(s3Reference) {
      logger.log("Retaining S3 source file reference.");
    } else {
